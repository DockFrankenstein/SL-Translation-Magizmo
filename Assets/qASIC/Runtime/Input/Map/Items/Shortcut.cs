...
{
    public class Shortcut : InputMapItem<bool>, ISerializableMapItem, ISupportsPrompts
    {
        ...
        public MapItemPromptData GetPromptData() =>
            new MapItemPromptData(new MapItemPromptData.PromptGroup(keys));
        ...
        public List<int> HasUnassignedPaths() =>
            keys
            .Select((x, i) => KeyTypeManager.GetProviderFromPath(x) == null ? i : -1)
            .Where(x => x != -1)
            .ToList();
    }
}