...
namespace qASIC.Input.Players
{
    public class InputPlayer
    {
        ...
        {
            ID = id;
            Input.Update.InputUpdateManager.OnUpdate += Update;
        }
        ...
        private IInputDevice _lastDevice = null;
        public IInputDevice LastDevice 
        {
            get => _lastDevice;
            private set
            {
                var changed = _lastDevice != value;
                _lastDevice = value;

                if (changed)
                    OnLastDeviceChanged?.Invoke(value);
            }
        }

        public event Action<IInputDevice> OnLastDeviceChanged;
        ...
        public bool DeviceExists(IInputDevice device) =>
